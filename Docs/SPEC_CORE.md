...
Monolith.uplugin
  ...
  MonolithConfig        — Config/INI resolution and search (6 actions, fixed surface) + the read-only `localization` namespace (4 actions — culture + StringTable inspection, registered AHEAD of the bEnableConfig gate)
  ...
  MonolithGAS           — Gameplay Ability System integration: abilities, attributes, effects, ASC, tags, cues, targets, input, inspection, scaffolding, UI attribute binding (~130+ actions, incl. 4 also aliased into `ui` namespace). Conditional on #if WITH_GBA. ALSO owns the `input` namespace (13 actions — Enhanced Input asset inspection + instanced modifier/trigger authoring; NOT GAS, and registered BEFORE the bEnableGAS gate so it survives GAS being switched off)
...
>
> **Ballpark, current:** **~1,400+ actions across 26 in-tree namespaces** (query `monolith_discover()` for the live figure — see Action Count Discipline in `monolith-release.md`). v0.23.0 added two namespaces: `input` (13, from `MonolithGAS`) and `localization` (4, from `MonolithConfig`).
...
| MonolithAnimation | chooser | 16 | **Two registration tiers.** The 10 authoring/edit actions are `WITH_CHOOSER`-gated; the 6 read-only actions added in 0.23.0 (`list_chooser_tables`, `get_chooser_table`, `list_chooser_columns`, `list_chooser_rows`, `list_chooser_references`, `validate_chooser_table` — PR #129 by @kunkunGames) are **NOT** gated: they read `UChooserTable` purely by reflection, with no `Chooser.h` include, so they register and answer identically with the plugin enabled or disabled (returning an explicit availability error, or `available=false` metadata for registry-only discovery, rather than synthesized data). `validate_chooser_table` is distinct from `validate_chooser` — it never compiles, mutates, saves or dirties a package. Gated set, registered from `MonolithAnimation`: `inspect_chooser`, `duplicate_chooser_tree`, `set_context_object_class`, `set_result_asset_reference`, `set_evaluate_chooser_result_reference` (rewrite the child table an EvaluateChooser/root/nested row points at; `duplicate_chooser_tree` also does two-pass nested `FEvaluateChooser`/`FNestedChooser` remapping + per-row `row_remap_report`), `validate_chooser`, plus the 2026-06-07 Motion Matching authoring set `create_chooser_table` / `add_chooser_column` (optional `enum_class` on `Enum` columns) / `add_chooser_row` / `set_chooser_cell` (typed predicate cell write, per-kind dispatch). `inspect_chooser` gained an optional `recursive` flag (2026-06-07) that resolves the full nested chooser tree (`child_tables[]` with per-row resolved asset path + row kind) and, in the 2026-06-10 introspection-gap pass (Gap 6, no count delta), a richer `columns` array (per-column `input_binding` chain + `is_input`) alongside the back-compat `column_types`, with per-row cell values gated behind `include_cells` — see [`specs/SPEC_MonolithAnimation.md` § Chooser Namespace](specs/SPEC_MonolithAnimation.md). Inert without the Chooser plugin (`Engine/Plugins/Chooser`) |
...
| MonolithConfig | localization | 4 | **New in v0.23.0.** `list_cultures`, `list_string_tables`, `get_string_table`, `validate_string_table`. Read-only culture + StringTable inspection, dispatcher annotated read-only + idempotent. **Registered AHEAD of the `bEnableConfig` gate** — these depend on nothing that gate controls, so they survive config authoring being switched off. Canonical mounted package paths only; never transacts, saves, mutates or dirties. StringTable *authoring* stays on `blueprint`. See [`specs/SPEC_MonolithConfig.md` § Localization Namespace](specs/SPEC_MonolithConfig.md) |
