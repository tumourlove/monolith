...
1. **The post-pass sentinels have no checkpoints, and most append rather than replace.** `dependencies`, `datatable_rows`, `configs`, `cpp_symbols`, `tags`/`tag_references` and the animation nodes are all written by sentinel indexers that enumerate the Asset Registry themselves rather than working from `assets` rows with ids, so ordinary per-asset deep checkpoints do not cover them. A resume repeats the whole post-pass phase, and because those indexers insert without clearing, an interruption during that phase can leave duplicate rows. `monolith_reindex force=true` clears them. `FLevelIndexer` is the bounded exception: it replaces `actors` rows per successfully processed world, preserves rows for worlds skipped under degraded pressure, and exposes partial/degraded completion through `level_index_state`, but it still re-walks every world because it has no checkpoint.
