...
An automatic full index records `full_index_state=in_progress` before work begins. Each deep-indexed asset writes a checkpoint keyed by `(package_path, saved_hash, indexer_name)` in the same transaction as its child data. Project-wide post-passes use `full_index_postpass.<indexer_name>` meta checkpoints. If shutdown, cancellation, a crash, or any indexing failure interrupts the run, startup bypasses `bDeferFirstTimeIndex`, refreshes the metadata pass, removes deleted packages, and queues only missing or hash-invalidated work. Added, removed, or hash-changed packages invalidate completed asset-derived post-pass checkpoints so project-wide derived data cannot go stale. Config, C++ symbol, and gameplay-tag passes always rerun after a restart because their inputs are not fully represented by asset package hashes.

The lifecycle covers every registered per-asset deep indexer automatically and requires a durable completion checkpoint from every enabled sentinel indexer. Sentinels with specialized orchestration run first; every remaining registered sentinel then runs through a generic compiler-idle transaction path. This includes optional and cross-module families such as GAS, MetaSound, and AI without introducing compile-time dependencies from `MonolithIndex` back to those modules. A final fail-closed check prevents any registered sentinel from being silently skipped. Animation remains a sentinel because it performs its own Asset Registry enumeration, but uses the same `(package_path, saved_hash, indexer_name)` checkpoint contract at one asset per compiler-idle editor tick. `last_full_index` is written atomically with recovery-state cleanup only after all enumerated metadata, deep-index, and enabled sentinel work succeeds; there is no fixed minimum project asset count. A manual forced full index intentionally discards these checkpoints.
