...
> **Deep Blueprint load invariant.** The full-index deep pass reads serialized Blueprint graphs without compiling them. Blueprint-derived assets are loaded with `LOAD_DisableCompileOnLoad`, matching Unreal's `CompileAllBlueprintsCommandlet`; non-Blueprint assets retain the normal `FAssetData::GetAsset` path. The Blueprint indexer reads generated-class defaults only when the saved class has `CLASS_CompiledFromBlueprint` and its existing default object has `RF_ClassDefaultObject`. A malformed Blueprint can therefore expose its saved graphs without driving the compiler queue or reinstancer into an editor-fatal assertion.

> **Exact-asset crash quarantine invariant.** Every deep-index package load, including full, incremental, and live indexing, is preceded by a committed marker for that asset alone. Its child-data transaction clears the marker only after indexing and package release complete. A process death therefore leaves one attributable path; the next run quarantines that path after the first interrupted attempt, persists it in the index metadata, writes `Saved/AssetsNeedingReview.log`, and continues around it. Quarantined paths are excluded from automatic incremental/live loads. On first open, marker format v2 clears counters, quarantine reports, and skip checkpoints left by the former batch-wide marker because those rows cannot identify a single culprit safely. `project get_stats` exposes `quarantined_assets`, `quarantined_asset_paths`, and `quarantine_report_path` while retaining `skipped_assets` / `skipped_asset_paths` as compatibility aliases. **Retry Fixed Assets** in Project Settings > Plugins > Monolith atomically clears the quarantined checkpoints, arms the full-index resume marker, and runs a hash-based resume so already healthy assets are not deep-indexed again. A second crash re-quarantines only the retried asset.
...
| `get_stats` | none | Row counts for all 13 tables + asset class breakdown (top 20) + exact deep-index quarantine fields (`quarantined_assets`, `quarantined_asset_paths`, and `quarantine_report_path`) + compatibility aliases (`skipped_assets`, `skipped_asset_paths`) + conditional `level_index_state` when level indexing completed partially or stopped under critical memory pressure |
...
**Exact-asset crash quarantine.** Immediately before each deep-index package load, `BumpDeepIndexAttempts` marks exactly that asset in its own committed transaction. The asset's child-data transaction clears the marker only after loading, indexing, package release, checkpointing, and commit all succeed. This ordering is the mechanism: under `journal_mode=DELETE`, a process death rolls back the unfinished child-data transaction but leaves the exact marker durable. A normal load or indexer failure rolls back partial child data and clears the marker because the process survived and the failure is already observable.

On the next run, one committed-but-unfinished marker quarantines that exact package. Healthy neighbours remain eligible regardless of their former batch membership. The quarantined asset is stamped with its current hash while its attempt counter remains armed, so even an empty or later-changing Asset Registry hash cannot silently requeue it. The path is logged at **Error**, persisted in `full_index_skipped_assets`, written to `Saved/AssetsNeedingReview.log`, and exposed by `project get_stats` through `quarantined_assets`, `quarantined_asset_paths`, and `quarantine_report_path`; `skipped_assets` and `skipped_asset_paths` remain compatibility aliases. Automatic incremental and live indexing skip the same paths.

**Retry.** **Retry Fixed Assets** in Project Settings > Plugins > Monolith atomically clears the quarantined assets' counters, deep checkpoints, and child data, clears the review list, marks the full index resumable, and starts a hash-based resume. Healthy assets keep their completed checkpoints; only missing or changed deep data is rebuilt. If a retried asset terminates the process again, the same exact path is quarantined again on the following run.

**Marker migration.** Batch-wide attempt counters from marker format v1 cannot identify a culprit. The first database open under marker format v2 clears those ambiguous counters, their affected deep checkpoints, and the legacy review list before stamping the new format. It does not quarantine any path based on ambiguous historical evidence.
...
- The animation post-pass gets **per-batch transactions** (bounding an interruption's loss to one batch instead of the whole pass) but no sentinel checkpoint. Exact quarantine covers animation assets when they pass through the ordinary deep-index queue; it does not checkpoint or attribute a later interruption inside the separate sentinel pass.
