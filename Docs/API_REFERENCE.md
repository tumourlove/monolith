...
`get_ordered_modules` / `add_module` support selector-based stages: `usage: "particle_event"` with `usage_id` or `handler_index`, and `usage: "particle_simulation_stage"` with `usage_id`, `stage_name`, or `stage_index`. Adding `GenerateDeathEvent` to `particle_update` auto-enables `requires_persistent_ids` on that emitter.
| HLSL / scripts | 2 | `create_module_from_hlsl`, `create_function_from_hlsl` |
**IMPORTANT:** Before writing custom HLSL, read `SPEC_MonolithNiagara.md` section on `create_module_from_hlsl` for complete HLSL body rules. HLSL is isolated (cannot directly access `Particles.*`), can use DI APIs, requires struct-wrapped functions, GPU code needs `#if GPU_SIMULATION`, outputs need default values. See `Niagara_Grid_HLSL_API_Reference.md` for Grid DI API.
...
`add_event_handler` only creates the handler and `ParticleEventScript` container. It returns `handler_index` and `usage_id`; callers must still add the matching `Receive<Event>` module when payload consumption is required. For inter-emitter handlers, `source_emitter` is now required and unresolved handlers are rejected.
