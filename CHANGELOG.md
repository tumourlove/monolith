...
- **A crashing asset no longer traps project indexing in a repeat-crash loop or takes healthy batch neighbours out with it.** Monolith now commits the exact package path immediately before each deep-index load and clears that marker only after the package has been indexed and released. If the editor process dies, the next run quarantines that one asset after the first interrupted attempt, writes `Saved/AssetsNeedingReview.log`, exposes the same paths through `project get_stats` (`quarantined_assets` / `quarantined_asset_paths`, with the existing `skipped_*` aliases retained), and continues the index around it. Incremental and live indexing also honor the quarantine. A one-time marker-format migration clears ambiguous counters and skip checkpoints written by the former batch-wide rule so healthy batch neighbours are not mislabeled. Project Settings > Plugins > Monolith now includes **Retry Fixed Assets**, which clears only the quarantined checkpoints and resumes a hash-based pass so fixed assets are retried without wiping healthy index data.
- **Deep Blueprint indexing no longer compiles malformed project Blueprints as a load side effect.** Blueprint, Widget Blueprint, and Animation Blueprint packages are loaded with Unreal's `LOAD_DisableCompileOnLoad` flag for read-only graph extraction, matching the engine's own `CompileAllBlueprintsCommandlet` loading pattern. Saved generated-class defaults are read only when the generated class and CDO carry the expected Blueprint/CDO flags. Broken assets can therefore contribute their serialized graph data without entering the Blueprint compiler's queued-request/reinstancing path and asserting the editor.
...
- **`project get_stats` reports assets skipped by deep indexing** — `skipped_assets` (count) and `skipped_asset_paths` (up to 50 paths). These compatibility fields are now aliases of the exact-asset quarantine described under Unreleased.
