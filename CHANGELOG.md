...
**Public action count: 1239** across 16 namespaces in the Monolith plugin proper. The four sibling plugins (`MonolithSteamBridge`, `MonolithISX`, `MonolithSubstance`, `MonolithClaudeDesignBridge`) live OUTSIDE `Plugins/Monolith/` and have independent release lifecycles — their actions (`steam`, `inventory`, `substance`, `claudedesign`) are NOT in this zip. For authoritative per-namespace breakdown see `Plugins/Monolith/Docs/SPEC_CORE.md` §12.
...
- **Public action count restated as 1239** (16 public namespaces). The previous "1277 → 1283 (+6 from Phase J)" framing didn't reflect the actual public surface in the release zip — it included pre-Phase-J counts that hadn't been audited against ground truth, and conflated optional / sibling-plugin actions with the public Monolith plugin proper. Sibling plugins (`MonolithSteamBridge` 28 actions, `MonolithISX` 158 actions, `MonolithSubstance` 26 actions, `MonolithClaudeDesignBridge` 11 actions) are NOT in this zip. The +6 Phase J adds (F8 + F18) and other in-release additions (CommonUI M0.5 +50 actions, PR #37 anim graph setter etc., F22 retrofit gating) all roll up into the 1239 figure.
