...
void FMonolithNiagaraActions::RegisterActions(FMonolithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_custom_hlsl_text"), TEXT("Read the Custom HLSL source text from a Niagara script's CustomHlsl node"),
		...
		FParamSchemaBuilder()
			.Required(TEXT("script_path"), TEXT("string"), TEXT("Niagara script asset path"))
...
FMonolithActionResult FMonolithNiagaraActions::HandleGetCustomHLSLText(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!HlslProp)
		return FMonolithActionResult::Error(TEXT("CustomHlsl property not found"));

	void* Addr = HlslProp->ContainerPtrToValuePtr<void>(TargetNode);
	FStrProperty* StrProp = CastField<FStrProperty>(HlslProp);
	if (!StrProp)
		return FMonolithActionResult::Error(TEXT("CustomHlsl property is not a string"));

	TSharedRef<FJsonObject> R = MakeShared<FJsonObject>();
	R->SetStringField(TEXT("script_path"), ScriptPath);
	R->SetStringField(TEXT("node_guid"), TargetNode->NodeGuid.ToString());
	R->SetStringField(TEXT("hlsl"), StrProp->GetPropertyValue(Addr));
	return SuccessObj(R);
}
...
FMonolithActionResult FMonolithNiagaraActions::HandleSetCustomHLSLText(const TSharedPtr<FJsonObject>& Params)
{
	...
	TargetNode->Modify();
	StrProp->SetPropertyValue(Addr, HlslText);
...
FMonolithActionResult FMonolithNiagaraActions::CreateScriptFromHLSL(const TSharedPtr<FJsonObject>& Params, ENiagaraScriptUsage Usage)
{
	...
	else
	{
		...
		// Connect the first input node (created at line 5196) to the first HlslNode input pin
		...
		// Connect first typed output pin on HlslNode to output node
		if (OutputIn)
		{
			for (UEdGraphPin* Pin : HlslNode->Pins)
			{
...
namespace
{
	struct FRendererAttributeHazardRule
	{
	...
	bool ContainsTokenInsensitive(const FString& Haystack, const TCHAR* Needle)
	{
		return Haystack.Contains(Needle, ESearchCase::IgnoreCase);
	}

	bool HasRendererClass(const TArray<UNiagaraRendererProperties*>& Renderers, const TCHAR* RendererClassName)
	{
	...
	bool HasConsumerModule(const TArray<FString>& ModuleScriptPaths, const TCHAR* ConsumerToken)
	{
		for (const FString& Path : ModuleScriptPaths)
		{
			if (ContainsTokenInsensitive(Path, ConsumerToken))
			{
				return true;
			}
		}
	...
	bool HasAttributeInitializationModule(const TArray<FString>& ParticleSpawnPaths, const TCHAR* FriendlyAttributeName)
	{
		for (const FString& Path : ParticleSpawnPaths)
		{
			if (ContainsTokenInsensitive(Path, TEXT("InitializeParticle")))
			{
				if (ContainsTokenInsensitive(Path, FriendlyAttributeName)
					|| ContainsTokenInsensitive(Path, TEXT("InitializeRibbon"))
					|| ContainsTokenInsensitive(Path, TEXT("InitializeMesh"))
					|| ContainsTokenInsensitive(Path, TEXT("InitializeSprite")))
				{
					return true;
				}
			}
...
FMonolithActionResult FMonolithNiagaraActions::HandleGetSystemDiagnostics(const TSharedPtr<FJsonObject>& Params)
{
	...
	for (const FNiagaraEmitterHandle& Handle : System->GetEmitterHandles())
	{
		...
		static const FRendererAttributeHazardRule HazardRules[] = {
		...
		for (const FRendererAttributeHazardRule& Rule : HazardRules)
		{
			if (!HasRendererClass(ED->GetRenderers(), Rule.RendererClassName))
			{
				continue;
			}

			const bool bHasConsumer = HasConsumerModule(ParticleSpawnPaths, Rule.ConsumerToken)
				|| HasConsumerModule(ParticleUpdatePaths, Rule.ConsumerToken);
			if (!bHasConsumer)
			{
				continue;
			}

			const bool bHasAttributeInit = HasAttributeInitializationModule(ParticleSpawnPaths, Rule.FriendlyAttributeName);
...
FMonolithActionResult FMonolithNiagaraActions::HandleSetStaticSwitchValue(const TSharedPtr<FJsonObject>& Params)
{
	...
	return SuccessStr(FString::Printf(TEXT("Static switch '%s' set to '%s' (raw='%s')"), *InputName, *DisplayValue, *ValStr));
}
